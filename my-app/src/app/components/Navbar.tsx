...
import { FaRocket, FaUserCircle } from "react-icons/fa"; // Tech-related icon and Profile icon
...
const Navbar = () => {
  ...
  const [balance, setBalance] = useState<string>('0 TWC'); // Initial balance
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      try {
        ...
        setBalance('15 TWC'); // Replace this with your logic to fetch balance
      } catch (error) {
  ...
  return (
    <nav className="absolute top-0 left-0 right-0 flex justify-between items-center p-6 bg-transparent z-50">
      ...
      >
        <FaRocket className="mr-2 text-yellow-300 text-2xl animate-bounce" />
        <a href="/">TweetCraft</a>
      </div>
